...
namespace CSharpQueue{
	public class LocalAssetBundleQueue : AsyncQueue<LocalAssetBundleQueue, LocalAssetBundleQueue.Request, LocalAssetBundleQueue.Response> {
		...
		protected override void ProcessRequest(Request request, Response response) {
			try {
				...
				FileWebRequest fileRequest = FileWebRequest.Create(request.assetURL) as FileWebRequest;
				fileRequest.CachePolicy = new System.Net.Cache.HttpRequestCachePolicy(HttpRequestCacheLevel.NoCacheNoStore);
				
				fileRequest.Timeout = 60;
				fileRequest.Headers["Accept-Encoding"] = "gzip";
				
				FileWebResponse fileResponse = fileRequest.GetResponse() as FileWebResponse;

				string assetBundlePath = basePath + "/" + request.assetName;
				System.Security.Cryptography.MD5 md5 = System.Security.Cryptography.MD5.Create();
				using (Stream responseStream = fileResponse.GetResponseStream(), fileStream = new FileStream(assetBundlePath, FileMode.Create)) {
