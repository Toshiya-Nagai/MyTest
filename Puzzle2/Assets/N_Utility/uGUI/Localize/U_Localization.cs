...
public class U_Localization : MonoBehaviour
{
	static U_Localization mInstance;
	...
	static public U_Localization instance
	{
		get
		{
			if (mInstance == null)
			{
				mInstance = Object.FindObjectOfType(typeof(U_Localization)) as U_Localization;

				if (mInstance == null)
				{
					GameObject go = new GameObject("_ULocalization");
					DontDestroyOnLoad(go);
					mInstance = go.AddComponent<U_Localization>();
				}
			}
			return mInstance;
		}
	...
	[HideInInspector] public string startingLanguage = "Japanese";
			...
			if (mLanguage != value)
			{
				if (!string.IsNullOrEmpty(value))
				{
					if (mDictionary.Count == 0)
					{
						...
						TextAsset txt = localizationHasBeenSet ? null : Resources.Load(value, typeof(TextAsset)) as TextAsset;
...
//		UIRoot.Broadcast("OnLocalize");
		#region add nagai
		//FIXME this code is slowly?
		var canvasList = GameObject.FindObjectsOfType<Canvas>();
		foreach(var buf in canvasList){
			buf.BroadcastMessage("OnLocalize",SendMessageOptions.DontRequireReceiver);
		}
		#endregion
	...
	/// </summary>
	///	NGUI Localize key		localize key set
	///	anLabelObj			: Localize enable object					
	///	anLoclizeKey		: set key
	/// </summary>
	static public void SetLocalizeKey(GameObject anLabelObj,string anLocalizeKey){
		anLabelObj.GetComponent<U_UILocalize>().key = anLocalizeKey;
		anLabelObj.SetActive(false);
		anLabelObj.SetActive(true);
	}
	//use NGUI ver upper 3.0  
	static public string GetLocalizeText(string anLocalizeKey,params object[] anSetParam){
		if(U_Localization.Exists(anLocalizeKey)){
			string temp = string.Format(U_Localization.Get(anLocalizeKey),anSetParam);
			return temp;
		}else{
			Debug.LogError("Not Key Found - " + anLocalizeKey);
			return "";
		}
	}
}
